...
class CSVDataMaker(DataMaker):
    ...
    def connect(self,verbose=0):
        ...
        lines = (hasattr(self.filename,'read') and self.filename or open(self.filename,'rb')).read()

        for bom, enc in self._boms:
            if lines.startswith(bom):
                #print('Found unicode byte order mark, stripping it')
                lines = lines[len(bom):]
                if enc!='UTF8': lines=lines.decode(enc).encode('utf8')
                break
        if b'\r\n' in lines: lines = lines.replace(b'\r\n',b'\n')
        if b'\r' in lines and b'\n' not in lines: lines = lines.replace(b'\r',b'\n')
        lines = lines.split(b'\n')
